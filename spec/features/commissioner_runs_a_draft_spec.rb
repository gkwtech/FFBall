...
feature "commissioner runs a draft" do
  ...
  scenario "and is able to assign players, pick-by-pick" do
    ...
    initial_roster_count = team.rosters.count
    ...
    select team.name , from: "pick[team_id][]"
    all(".hidden-player-id").last.set player.id
    click_button "Sold"
    expect(team.rosters.count).to eq(initial_roster_count + 1)
  end
  ...
  scenario "but is unable to assign already-assigned players" do
    ...
    visit new_league_draft_path(league)
    click_button("Submit")
    expect(page.find(".player-name")).to have_content(player.first_name)
    fill_in "Price", with: "1"
    select team.name , from: "pick[team_id][]"
    all(".hidden-player-id").last.set player.id
    click_button "Sold"
    expect(team.rosters.count).to eq(initial_roster_count + 1)
    expect{page.find(".player-name")}.to raise_error(Capybara::ElementNotFound)
  end
  ...
  scenario "and is able to view draft history as it occurs" do
    ...
    expect(team.rosters.count).to eq(initial_roster_count + 1)
    expect{page.find(".player-name")}.to raise_error(Capybara::ElementNotFound)
    page.should have_css(".draft-header-teams-player-name", :text => player.first_name)
  end
